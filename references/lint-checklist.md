...
- For `System/`, only check configuration health: `System/obsiwiki.toml` exists when the vault has adopted the locator protocol, key schema files are readable, agent adapters follow `System/Schema/`, and vault-local schema customizations are not silently overwritten.
